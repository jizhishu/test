...
class Product(item.Item):
    ...
    def searchByCategory(self,categoryNodeID,sortItem,direction,skipNum,limitNum=20):
        '''fun: search product list by category
        '''
        direct = None
        if cmp(direction,"ASC")==0:
            direct = 1
        else:
            direct = -1
        result = self._item_connect.search("ancestor_list",categoryNodeID).sort(sortItem,direct)
        maxcount = result.count()
        resultlist = result.skip(skipNum).limit(limitNum)
        r = {
             'maxlength':maxcount,
    ...
    def searchCountByCategory(self,categoryNodeID):
        '''fun: get the count of product in one category
        '''
        return self._item_connect.search("ancestor_list",categoryNodeID).count()
